...
void LSM303DLH_Acc_Read_Acc(s16* out)
{
  ...
  if(!(crtl4 & 0x40))
  {
    ...
    case 0x00:
      for(i=0; i<3; i++)
      {
        out[i]=(s16)(LSM_Acc_Sensitivity_2g*((u16)buffer[2*i+1] << 8) + buffer[2*i]);
      }
      break;
    case 0x10:
      for(i=0; i<3; i++)
        out[i]=(s16)(LSM_Acc_Sensitivity_4g*((u16)buffer[2*i+1] << 8) + buffer[2*i]);
      break;
    case 0x30:
      for(i=0; i<3; i++)
        out[i]=(s16)(LSM_Acc_Sensitivity_8g*((u16)buffer[2*i+1] << 8) + buffer[2*i]);
    }
  }
  else
  {
    ...
    case 0x00:
      for(i=0; i<3; i++)
        out[i]=((s16)(LSM_Acc_Sensitivity_2g*(u16)buffer[2*i] << 8) + buffer[2*i+1])/16;
      break;
    case 0x10:
      for(i=0; i<3; i++)
        out[i]=((s16)(LSM_Acc_Sensitivity_4g*(u16)buffer[2*i] << 8) + buffer[2*i+1])/16;
      break;
    case 0x30:
      for(i=0; i<3; i++)
        out[i]=(s16)(LSM_Acc_Sensitivity_8g*((u16)buffer[2*i] << 8) + buffer[2*i+1])/16;
      break;
    }
  }
}
...
void LSM303DLH_Magn_Read_Magn(s16* out)
{
  ...
  LSM303DLH_I2C_BufferRead(LSM_M_I2C_ADDRESS, &buffer[0], LSM_M_OUT_X_L_ADDR, 1);
  LSM303DLH_I2C_BufferRead(LSM_M_I2C_ADDRESS, &buffer[1], LSM_M_OUT_X_H_ADDR, 1);
  LSM303DLH_I2C_BufferRead(LSM_M_I2C_ADDRESS, &buffer[2], LSM_M_OUT_Y_L_ADDR, 1);
  LSM303DLH_I2C_BufferRead(LSM_M_I2C_ADDRESS, &buffer[3], LSM_M_OUT_Y_H_ADDR, 1);
  LSM303DLH_I2C_BufferRead(LSM_M_I2C_ADDRESS, &buffer[4], LSM_M_OUT_Z_L_ADDR, 1);
  LSM303DLH_I2C_BufferRead(LSM_M_I2C_ADDRESS, &buffer[5], LSM_M_OUT_Z_H_ADDR, 1);
